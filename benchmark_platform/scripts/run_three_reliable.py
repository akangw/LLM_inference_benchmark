...
def main():
    ...
    for sc in RUNS:
        ...
        for line in out.splitlines():
            if any(
                k in line
                for k in ["渲染", "探活", "探通", "FAIL", "REJECT", "ERROR", "就绪"]
            ):
                if not any(
                    k in line for k in ["Warning", "_owner", "Diffusers", "onnxruntime"]
                ):
                    print(line, flush=True)
                    with open(LOG, "a") as f:
                        f.write(line + "\n")
        ...
        log("✅ 服务就绪，开始 benchmark (150 req / 并发 5)...")
        rc_b, out_b = run(
            [
                PY,
                CLI,
    ...
    print("网页: http://<ip>:8088/leaderboard/chat_completions  (15s 自动刷新)")
