...
页面导航:`/submit` 提交 · `/jobs` 列表 · `/jobs/{id}` 详情 ·
`/leaderboard` · `/leaderboard/chat_completions` · `/leaderboard/completions`。
...
- **进榜条件**(全满足):`public_leaderboard` + `llmperf_550_150` + 150 请求 + 并发 5 +
  max_tokens 150 + temp 0 + top_p 1 + stream 真生效 + 完整结束 + err≤1% + succ≥99%。
  smoke / stress 永不进榜,但仍出全套 JSON/HTML/CSV 报告并可在 `/jobs/{id}` 查看。

---

## 产物位置

每个任务在 `runs/<job_id>/`:`config.json`、`stdout.log`、`stderr.log`、
`raw_result.json`、`parsed_result.json`、`per_request_metrics.jsonl`、
